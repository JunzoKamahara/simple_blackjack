...
class BlackjackPlayer:
    ...
    def __init__(self, balance, stop_balance=stop_balance, unit_bet=unit_bet, hit_threshold=hit_threshold):
        '''
        Parameters:
        ----------
        balance: int
        ...
        self.stop_balance = stop_balance # プレイヤーがゲームを終了する残高
        self.unit_bet = unit_bet # プレイヤーが賭ける単位金額
        self.hit_threshold = hit_threshold # プレイヤーがヒットする閾値
        self.hand = []
    ...
    def place_bet(self, balance):
        ...
        '''
        self.balance = balance
        if balance > self.stop_balance:
            return 0
        else:
            if balance > self.unit_bet:
                bet_amount = self.unit_bet
            else:
                bet_amount = balance
            return bet_amount
    ...
    def input(self, message):
        ...
        '''
        if Card.sum(self.hand) < self.hit_threshold:
            choice = "hit"
        else:
            choice = "stand"
        return choice
